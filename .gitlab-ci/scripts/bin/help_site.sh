...
mr_url=$(glab mr create -s release_notes_"$CI_COMMIT_TAG"_"$CIPHERGUARD_FLAVOUR" -b master -d ":robot: Release notes for $CI_COMMIT_TAG $CIPHERGUARD_FLAVOUR" -t "Release notes for $CIPHERGUARD_FLAVOUR $CI_COMMIT_TAG" --push --repo "khulnasoft/cipherguard-help" | grep 'https://gitlab.com/khulnasoft/cipherguard-help/-/merge_requests/')
cd -
